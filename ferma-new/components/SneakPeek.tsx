...
const SneakPeek: React.FC<SneakPeekProps> = ({ content }) => {
  ...
  const images = [
    "https://ferma.ivins.lv/ferma-scrn-1.png",
    "https://ferma.ivins.lv/ferma-scrn-2.png",
    "https://ferma.ivins.lv/ferma-scrn-3.png"
  ];
          ...
          {images.map((img, idx) => (
            <div key={idx} className="relative group rounded-3xl overflow-hidden shadow-lg border border-stone-200 aspect-[4/3]">
              <img 
                src={img} 
                alt="Sneak peek" 
                className="w-full h-full object-cover filter blur-md brightness-75 group-hover:scale-110 transition-transform duration-700" 
              />
